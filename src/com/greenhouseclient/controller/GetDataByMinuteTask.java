 ...
 *
 */
public class GetDataByMinuteTask extends BaseTask
{
	private static final int TASK_TAG = TaskConstants.GET_DATA_BY_MINUTE_TASK;
	private long startTime ;
	private long endTime ;
	public GetDataByMinuteTask(Handler handler, Context _context)
	{
		super(handler, _context);
		endTime = new Date().getTime();
		startTime = endTime-Constants.HOUR_TIME;
	}
	...
	public void run()
	{
		...
		for(int i =0; i<GreenHouseApplication.gatawayListBean.gwList.length ;i++)
		{
			...
			dataRequestBean = (DataRequestBean) httpManager.requestServer(Constants.GetDataByMinute_Url, dataRequestBean, true);
			//开始封装数据
			//先声明网关
			Map<Integer,ArrayList<MQTT_DetectorDataBean>> gwdataMap =DataKeeper.All_GW_Collections_Minute.get(GreenHouseApplication.gatawayListBean.gwList[i].gwid);
			if(gwdataMap == null)
			{
				//如果木有这个网关，就new一个
...
//				gwdataMap.put(dataRequestBean.dataList[0].detectorDatas[0].did, detectorList);
			...
			DataKeeper.All_GW_Collections_Minute.put(GreenHouseApplication.gatawayListBean.gwList[i].gwid, gwdataMap);
		}
