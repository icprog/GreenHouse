...
public class ShowDetectorActivity extends BaseActivity
{
	...
	protected void initViews()
	{
		...
		showView = (ViewGroup) getLayoutInflater().inflate(R.layout.detector_center_activity, null);
		mMenuImageButton.setOnClickListener(this);
		mViewPager = (ViewPager) showView.findViewById(R.id.centerview_viewpager);
	...
	protected void onResume()
	{
		...
		if (GreenHouseApplication.gatawayListBean.gwList.length != 0)
		{
			...
			// for (int i = 0; i <
			// GreenHouseApplication.gatawayListBean.gwList.length; i++)
			// {
			View itemView = initPagerItemView(gwid);
	...
	{
		View childView = getLayoutInflater().inflate(R.layout.showdata_view, null);
		childView.setTag(gwid);
		TextView gwNameTv = (TextView) childView.findViewById(R.id.showdataview_gwid_textview);
		// gwNameTv.setText("网关ID：" + gwid);
		gwNameTv.setText("探测器：" + "TEST_DETECTOR_1");

		MAChart temp_machart = (MAChart) childView.findViewById(R.id.temp_machart);
		MAChart humi_machart = (MAChart) childView.findViewById(R.id.humi_machart);
		MAChart beam_machart = (MAChart) childView.findViewById(R.id.beam_machart);
		...
		Spinner sp = (Spinner) childView.findViewById(R.id.select_showdata_type);
		...
		sp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener()
		{
			...
			public void onItemSelected(AdapterView<?> parent, View view, int position, long id)
			{
				...
				case 0:
				{
					...
					MAChart temp_machart = (MAChart) ViewPagerItem.findViewById(R.id.temp_machart);
					MAChart humi_machart = (MAChart) ViewPagerItem.findViewById(R.id.humi_machart);
					MAChart beam_machart = (MAChart) ViewPagerItem.findViewById(R.id.beam_machart);
					initMACharts(temp_machart, TYPE_HOUR, TEMP, gwid);
					initMACharts(humi_machart, TYPE_HOUR, HUMI, gwid);
					initMACharts(beam_machart, TYPE_HOUR, BEAM, gwid);
	...
	{
		TextView temp_tv = (TextView) contentView.findViewById(R.id.temp_tv);
		TextView humi_tv = (TextView) contentView.findViewById(R.id.humi_tv);
		TextView beam_tv = (TextView) contentView.findViewById(R.id.beam_tv);
		if (DataKeeper.All_GW_Collections_Minute.get(gwid) != null && DataKeeper.All_GW_Collections_Minute.size() > 0)
		{
			List<MQTT_DetectorDataBean> list = DataKeeper.All_GW_Collections_Minute.get(gwid).get(DEFAULT_DECETOR);
			if (list != null && list.size() > 0)
			{
//				MQTT_DetectorDataBean currentData = list.get(list.size() - 1);
...
//
		...
		mMQTTHandler = new Handler()
		{
			...
			public void handleMessage(Message msg)
			{
				...
				try
				{
					...
					initMACharts((MAChart) childView.findViewById(R.id.temp_machart), TYPE_HOUR, TEMP, gwid);
					initMACharts((MAChart) childView.findViewById(R.id.humi_machart), TYPE_HOUR, HUMI, gwid);
					initMACharts((MAChart) childView.findViewById(R.id.beam_machart), TYPE_HOUR, BEAM, gwid);
	...
	private class MySpinnerAdapter extends BaseAdapter
	{
		...
		{
			View content_view = getLayoutInflater().inflate(R.layout.spinner_item, null);
	...
	public void initMACharts(MAChart machart, int TIME_TYPE, int dataTYPE, int gwid)
	{
		...
		List<MQTT_DetectorDataBean> listDataBean = null;
		switch (TIME_TYPE)
		{
		case TYPE_HOUR:
			xtitle.add("0分");
			...
			if (DataKeeper.All_GW_Collections_Minute.get(gwid) != null)
				listDataBean = DataKeeper.All_GW_Collections_Minute.get(gwid).get(DEFAULT_DECETOR);
			break;
		case TYPE_DAY:
			xtitle.add("0时");
			xtitle.add("1");
			...
			if (DataKeeper.All_GW_Collections_Hour.get(gwid) != null)
				...
				listDataBean = DataKeeper.All_GW_Collections_Hour.get(gwid).get(DEFAULT_DECETOR);
			break;
		case TYPE_WEEK:
			xtitle.add("周一");
			xtitle.add("周二");
			...
			if (DataKeeper.All_GW_Collections_Date.get(gwid) != null)
				...
				listDataBean = DataKeeper.All_GW_Collections_Date.get(gwid).get(DEFAULT_DECETOR);
			break;
		default:
			break;
		}
		...
		if (listDataBean != null)
		{
			...
			case TEMP:
			{
				Iterator<MQTT_DetectorDataBean> iterator = listDataBean.iterator();
				while (iterator.hasNext())
				{
					MQTT_DetectorDataBean greenHouseBaseDataBean = (MQTT_DetectorDataBean) iterator.next();
					if (greenHouseBaseDataBean.temperature < -50)
					{
						data.add(lastData);
					} else
					{
						data.add((float) greenHouseBaseDataBean.temperature);
						lastData = (float) greenHouseBaseDataBean.temperature;
					}
				}
			...
			case HUMI:
			{
				...
				{
					MQTT_DetectorDataBean greenHouseBaseDataBean = (MQTT_DetectorDataBean) iterator.next();
					if (greenHouseBaseDataBean.humidity < -50)
					{
						data.add(lastData);
					} else
					{
						data.add((float) greenHouseBaseDataBean.humidity);
						lastData = (float) greenHouseBaseDataBean.humidity;
					}
				}
				machart.setMaxValue(100);
			}
			...
			case BEAM:
			{
				...
				{
					MQTT_DetectorDataBean greenHouseBaseDataBean = (MQTT_DetectorDataBean) iterator.next();
					if (greenHouseBaseDataBean.beam < -50)
					{
						data.add(lastData);
					} else
					{
						data.add((float) greenHouseBaseDataBean.beam);
						lastData = (float) greenHouseBaseDataBean.beam;
					}
				}
